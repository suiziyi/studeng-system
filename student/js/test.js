...
$(function(){
    ...
    $('.prog .div_h2').html('<h2 style="color:red;font-size: 25px;display: inline;"><b>\n' +
        '                                           <span>'+chaName+'</span>——测验</b>\n' +
        '                                        </h2>');
    $.ajax({
        type: 'get',
        url: "http://111.230.31.212/judgement/examination/generateTestPaper",
        dataType: "json",
        data: {
...
//初始代码
    ...
    var ts = parseInt($.session.get('timeid'))*60;//计算剩余的毫秒数
    // var ts=10;
    function timer()
    {
...
//编程
        ...
        $('.input_hidden input').each(function () {
            program_data = {
                "studentProgram":$(this).attr('value'),
                "exeId":parseInt($(this).attr('name'))
            };
